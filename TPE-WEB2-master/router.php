...
switch ($params[0]) {
    ...
    case "addEditorial":
        sessionStatus($res);
        $editorialController->addEditorial();
        break;
    ...
    case "editEditorial":
        sessionStatus($res);
        if (isset($params[1])) {
            $editorialController->showEditEditorialForm($params[1]); // Mostrar formulario de edición
        } else {
            echo " La ID de la editorial no fue proporcionada para edición";
        }
        break;
    case "updateEditorial":
        sessionStatus($res);
        $editorialController->updateEditorial(); // Actualizar editorial
        break;
}
